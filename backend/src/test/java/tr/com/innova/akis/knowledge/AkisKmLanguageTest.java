...
class AkisKmLanguageTest {
    ...
    @Test void versionTwoOptionsAreTypedAndCanonical() {
        var program=parse(example(Kind.IKM));
        assertEquals(VERSION,program.language());
        assertEquals(List.of("WRITE_MODE","KEY_COLUMNS","TRUNCATE_TARGET","ORACLE_HINT"),program.options().stream().map(Option::key).toList());
    ...
    @Test void quotedDefaultsPreserveSpacesQuotesAndLiteralYok() {
        ...
        assertEquals("FULL(T) PARALLEL(4)",options.get(3).defaultValue());
        assertEquals("Müşteri \"aktif\"",options.get(4).defaultValue());
        assertEquals("YOK",options.get(5).defaultValue());
    }
