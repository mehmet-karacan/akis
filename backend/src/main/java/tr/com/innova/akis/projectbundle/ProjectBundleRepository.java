...
class ProjectBundleRepository {
    ...
    void importPortableTopology(long projectId, JsonNode topology) {
        ...
        for(JsonNode item:array(topology,"physicalSchemas")){long id=jdbc.sql("insert into akis.fiziksel_sema(proje_id,baglanti_id,kod,ad,sema_adi) values(:p,:b,:c,:n,:s) returning id").param("p",projectId).param("b",connections.get(text(item,"connectionCode"))).param("c",text(item,"code")).param("n",text(item,"name")).param("s",text(item,"schemaName")).query(Long.class).single();physical.put(text(item,"code"),id);}
