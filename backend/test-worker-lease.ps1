...
try {
    ...
    & $maven -pl backend "-Dtest=JdbcRunLeaseStoreIT,JdbcRunReconciliationStoreIT,JdbcPinnedSchemaSnapshotStoreIT,JdbcRunExecutionTransitionStoreIT,WorkerAcknowledgementLossMigrationIT,LeaseReaperSafetyMigrationIT" test
    if ($LASTEXITCODE -ne 0) {
