...
DO $$ DECLARE actual BIGINT; required_column TEXT; BEGIN
 ...
 IF actual<>32 THEN RAISE EXCEPTION 'Beş grup sonunda 32 tablo bekleniyordu; bulunan %.',actual; END IF;
 FOREACH required_column IN ARRAY ARRAY['id','uuid','olusturulma_zamani','olusturan_kullanici_id','guncellenme_zamani','guncelleyen_kullanici_id','versiyon_no'] LOOP
  ...
  IF actual<>32 THEN RAISE EXCEPTION 'Ortak kolon % yalnız % tabloda.',required_column,actual; END IF;
 END LOOP;
END $$;
SELECT 'AKIS_SCENARIOS_OK' AS verification_result;
