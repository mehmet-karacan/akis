...
test('catalogs share full-width layout and logical records edit and delete without navigation', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Panel Test', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Updated Panel', exact: true }).click()
  await dialog.getByRole('button', { name: 'Delete', exact: true }).click()
  await page.locator('.ant-popconfirm').getByRole('button', { name: 'Delete', exact: true }).click()
  await expect(dialog).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Updated Panel', exact: true })).toHaveCount(0)
  await expect(page).toHaveURL(/\/project\/logical-schemas$/)
  ...
  await expect(page.locator('.connection-record-card').first()).toBeVisible()
  await expectNoHorizontalOverflow(page)
