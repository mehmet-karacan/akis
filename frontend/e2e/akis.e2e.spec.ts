...
test.describe('AKIŞ critical browser journeys', () => {
  ...
  test('keeps package authoring focused on the canvas and opens linked objects on double click', async ({ page }) => {
    ...
    await expect(page.locator('.sidebar-type-cluster').filter({ hasText: 'Packages' }).first()).toBeVisible()
    ...
    await expect(page.locator('.package-component-picker input')).toBeVisible()
