...
export function SchemaContextDetailPage({ kind, recordUuid, embedded = false }: { kind: 'logical' | 'environment'; recordUuid?: string; embedded?: boolean }) {
  const params = useParams(); const projectUuid = useCurrentProjectUuid(); const uuid = recordUuid ?? (kind === 'logical' ? params.logicalSchemaUuid ?? '' : params.environmentUuid ?? ''); const { t, i18n } = useTranslation()
  ...
  const missing = i18n.language.startsWith('tr') ? 'Tanımlanmadı' : 'Not configured'
  return <section className="page-stack schema-page">{!embedded && <Link className="connection-back-link" to={`/projects/${projectUuid}/${backPath}`}><ArrowLeft size={16} />{kind === 'logical' ? t('schemas.logicalTitle') : t('schemas.environmentsTitle')}</Link>}{!embedded && <PageHeader eyebrow={definition.code} title={definition.name} description={kind === 'logical' ? (definition as LogicalSchema).description ?? t('schemas.logicalDetailHint') : t('schemas.environmentDetailHint')} />}<section className="connection-detail-section"><h2>{t('schemas.resolvedTargets')}</h2>{rows.length === 0 ? <AsyncState state="empty" compact title={t('schemas.notMapped')} /> : <div className="schema-list-table"><DataGrid><thead><tr><th>{kind === 'logical' ? t('schemas.environment') : t('schemas.logicalSchema')}</th><th>{t('connections.connection')}</th><th>{t('schemas.physicalSchema')}</th></tr></thead><tbody>{rows.map((binding) => { const schema = physical.find((item) => item.uuid === binding.physicalSchemaUuid); const connection = connections.find((item) => item.uuid === schema?.connectionUuid); const context = kind === 'logical' ? environments.find((item) => item.uuid === binding.environmentUuid) : logical.find((item) => item.uuid === binding.logicalSchemaUuid); return <tr key={binding.uuid}><td><strong>{context?.name ?? missing}</strong></td><td>{connection?.name ?? missing}</td><td><code>{schema?.schemaName ?? missing}</code></td></tr> })}</tbody></DataGrid></div>}</section></section>
}
