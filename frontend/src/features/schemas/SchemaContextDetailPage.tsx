...
function SchemaContextDetailPage({ kind }: { kind: 'logical' | 'environment' }) {
  ...
  return <section className="page-stack schema-page"><Link className="connection-back-link" to={`/projects/${projectUuid}/${backPath}`}><ArrowLeft size={16} />{kind === 'logical' ? t('schemas.logicalTitle') : t('schemas.environmentsTitle')}</Link><PageHeader eyebrow={definition.code} title={definition.name} description={kind === 'logical' ? (definition as LogicalSchema).description ?? t('schemas.logicalDetailHint') : t('schemas.environmentDetailHint')} /><section className="connection-detail-section"><h2>{t('schemas.resolvedTargets')}</h2>{rows.length === 0 ? <AsyncState state="empty" compact title={t('schemas.notMapped')} /> : <div className="schema-list-table"><table><thead><tr><th>{kind === 'logical' ? t('schemas.environment') : t('schemas.logicalSchema')}</th><th>{t('connections.connection')}</th><th>{t('schemas.physicalSchema')}</th><th>{t('schemas.connectionRevision')}</th></tr></thead><tbody>{rows.map((binding) => { const schema = physical.find((item) => item.uuid === binding.physicalSchemaUuid); const connection = connections.find((item) => item.uuid === schema?.connectionUuid); const version = Object.values(versions).flat().find((item) => item.uuid === binding.connectionVersionUuid); const context = kind === 'logical' ? environments.find((item) => item.uuid === binding.environmentUuid) : logical.find((item) => item.uuid === binding.logicalSchemaUuid); return <tr key={binding.uuid}><td><strong>{context?.name ?? '—'}</strong></td><td>{connection?.name ?? '—'}</td><td><code>{schema?.schemaReference ?? '—'}</code></td><td>r{version?.versionNumber ?? '—'}</td></tr> })}</tbody></table></div>}</section></section>
}
