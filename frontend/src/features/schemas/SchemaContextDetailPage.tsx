...
export function SchemaContextDetailPage({ kind, recordUuid, embedded = false }: { kind: 'logical' | 'environment'; recordUuid?: string; embedded?: boolean }) {
  const params = useParams(); const projectUuid = useCurrentProjectUuid(); const uuid = recordUuid ?? (kind === 'logical' ? params.logicalSchemaUuid ?? '' : params.environmentUuid ?? ''); const { t } = useTranslation()
