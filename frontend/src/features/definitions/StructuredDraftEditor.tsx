...
export function StructuredDraftEditor({ type, value, onChange }: Props) {
  ...
  if (type === 'USER_FUNCTION') {
    ...
    return <div className="structured-draft-form">
      <label><span>{t('returnType')}</span><select value={String(content.returnType ?? 'STRING')} onChange={(event) => set('returnType', event.target.value)}><option>STRING</option><option>NUMBER</option><option>DATE</option><option>BOOLEAN</option></select></label>
      ...
      <section className="structured-draft-wide structured-list-editor"><header><strong>{t('parameters')}</strong><button className="definition-button definition-button--quiet" type="button" onClick={() => set('parameters', [...parameters, { name: `PARAM_${parameters.length + 1}`, dataType: 'STRING' }])}><Plus size={15} />{t('addParameter')}</button></header>{parameters.map((parameter, index) => <div key={index}><input aria-label={t('parameterName')} value={String(parameter.name ?? '')} onChange={(event) => set('parameters', parameters.map((item, position) => position === index ? { ...item, name: event.target.value } : item))} /><select aria-label={t('dataType')} value={String(parameter.dataType ?? 'STRING')} onChange={(event) => set('parameters', parameters.map((item, position) => position === index ? { ...item, dataType: event.target.value } : item))}><option>STRING</option><option>NUMBER</option><option>DATE</option><option>BOOLEAN</option></select><button className="definition-icon-button" type="button" aria-label={t('remove')} onClick={() => set('parameters', parameters.filter((_, position) => position !== index))}><Trash2 size={15} /></button></div>)}</section>
    </div>
  }
