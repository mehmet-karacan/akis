...
export function MappingKmOptions({ projectUuid, value, onChange }: { projectUuid: string; value: MappingContent; onChange: (value: MappingContent) => void }) {
  ...
  return <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
    ...
    <Form layout="vertical" component="div"><Row gutter={[16, 8]}>
      <Col xs={24} lg={6}><Form.Item label={tr ? 'Çalışma Mantıksal Şeması' : 'Staging Logical Schema'} htmlFor={`${id}-schema`}>
      ...
      {([['loading', 'LKM'], ['checking', 'CKM'], ['integration', 'IKM']] as const).map(([role, kind]) => <Col key={role} xs={24} lg={6}>
        <Form.Item label={`${kind}${role === 'checking' ? (tr ? ' (İsteğe Bağlı)' : ' (Optional)') : ''}`} htmlFor={`${id}-${role}`}>
