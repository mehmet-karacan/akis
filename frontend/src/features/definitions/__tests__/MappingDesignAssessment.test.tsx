...
it('checks only on demand and labels physical execution as unverified', async () => {
  ...
  expect(JSON.parse(request.mock.calls[0]![1]!.body as string).content.sources[0]).not.toHaveProperty('name')
})
...
it('offers explicit legacy upgrade and shows rejection without a success badge', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Upgrade to the New Interface Contract' }))
