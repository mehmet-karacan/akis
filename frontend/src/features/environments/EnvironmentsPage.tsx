...
export function EnvironmentsPage() {
  ...
  return <section className="page-stack schema-page"><PageHeader eyebrow={t('schemas.eyebrow')} title={t('schemas.environmentsTitle')} description={t('schemas.environmentsDescription')} actions={<Button tone="primary" icon={<Plus size={16} />} onClick={() => setOpen(true)}>{t('schemas.addEnvironment')}</Button>} />{error ? <div className="error-banner" role="alert">{error}</div> : null}{loading ? <AsyncState state="loading" title={t('common.loading')} /> : items.length === 0 ? <AsyncState state="empty" title={t('schemas.noEnvironments')} /> : <div className="schema-list-table"><table><thead><tr><th>{t('schemas.name')}</th><th>{t('schemas.code')}</th></tr></thead><tbody>{items.map((item) => <tr key={item.uuid}><td><Link className="schema-name-link" to={`/projects/${projectUuid}/environments/${item.uuid}`}>{item.name}</Link></td><td><code>{item.code}</code></td></tr>)}</tbody></table></div>}<Dialog open={open} title={t('schemas.addEnvironment')} closeLabel={t('common.close')} busy={busy} onClose={() => setOpen(false)}><form onSubmit={(event) => void create(event)}><label>{t('schemas.name')}<input name="name" required /></label><label>{t('schemas.code')}<input name="code" pattern="[A-Za-z][A-Za-z0-9_]{0,99}" required /></label><p className="form-note">{t('schemas.environmentSimpleHint')}</p><footer><Button type="button" onClick={() => setOpen(false)}>{t('common.cancel')}</Button><Button type="submit" tone="primary" busy={busy}>{t('schemas.create')}</Button></footer></form></Dialog></section>
}
