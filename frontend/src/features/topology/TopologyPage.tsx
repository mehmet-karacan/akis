  ...
  return (
    <main className="topology-page">
      ...
      {form && <Drawer title={formTitle[form]} closeLabel={tr('close')} onClose={() => setForm(null)}>
        ...
        {form === 'version' && selectedConnection && <form className="topology-form" onSubmit={(event) => void submit('version', event, (data) => topologyApi.createVersion(projectUuid, selectedConnection.uuid, { driverReference: textValue(data, 'driverReference'), host: textValue(data, 'host'), port: Number(textValue(data, 'port')), serviceName: optionalValue(data, 'serviceName'), sid: optionalValue(data, 'sid'), tlsMode: textValue(data, 'tlsMode'), policyVersion: Number(textValue(data, 'policyVersion') || 1), secretReferenceUuid: textValue(data, 'secretReferenceUuid'), secretRole: textValue(data, 'secretRole') }), async () => { setVersions(await topologyApi.listVersions(projectUuid, selectedConnection.uuid)) })}><Field label={tr('driver')} name="driverReference"><input name="driverReference" defaultValue="oracle.jdbc.OracleDriver" required /></Field><Field label={tr('host')} name="host" /><div className="topology-form-row"><Field label={tr('port')} name="port"><input name="port" type="number" min="1" max="65535" defaultValue="1521" required /></Field><Field label={tr('tlsMode')} name="tlsMode"><select name="tlsMode" defaultValue="DISABLED" required><option value="DISABLED">DISABLED</option><option value="REQUIRED">REQUIRED</option></select></Field></div><Field label={tr('serviceName')} name="serviceName" optional /><Field label={tr('sid')} name="sid" optional /><Field label={tr('secret')} name="secretReferenceUuid"><select name="secretReferenceUuid" defaultValue="" required><option value="" disabled>—</option>{resources.secrets.map((item) => <option key={item.uuid} value={item.uuid}>{item.name} · {item.provider}</option>)}</select></Field><Field label={tr('secretRole')} name="secretRole"><select name="secretRole" defaultValue="KIMLIK" required><option value="KIMLIK">KIMLIK</option></select></Field><Field label={tr('policyVersion')} name="policyVersion"><input name="policyVersion" type="number" min="1" defaultValue="1" required /></Field><Submit busy={busy === 'version'} c={c} /></form>}
