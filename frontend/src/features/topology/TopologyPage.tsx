  ...
  return (
    <main className="topology-page">
        ...
        <>
          {tab === 'connections' && (
            <div className="topology-columns topology-columns--master-detail">
              ...
              <section className="topology-panel">
                <PanelHeading icon={<ServerCog />} title={tr('versions')} count={versions.length} action={selectedConnection && <button className="topology-button topology-button--small" onClick={() => setForm('version')}><Plus />{tr('addVersion')}</button>} />
                {!selectedConnection ? <Empty>{tr('chooseConnection')}</Empty> : versionsLoading ? <div className="topology-loading"><LoaderCircle className="is-spinning" />{tr('loading')}</div> : versions.length === 0 ? <Empty>{tr('noItems')}</Empty> : (
                  <div className="topology-version-list">{versions.map((item) => <button key={item.uuid} aria-pressed={selectedVersionUuid === item.uuid} onClick={() => setSelectedVersionUuid(item.uuid)}><span>v{item.versionNumber}</span><strong>{item.host}:{item.port}</strong><small>{item.serviceName || item.sid || item.databaseName || '—'}</small></button>)}</div>
                )}
            ...
            <div className="topology-columns topology-columns--three">
              ...
              <section className="topology-panel topology-panel--wide">
                ...
                <form className="topology-discovery" onSubmit={(event) => void runDiscovery(event)}>
                  ...
                  <label><span>{tr('version')}</span><select value={selectedVersionUuid} onChange={(event) => setSelectedVersionUuid(event.target.value)} required disabled={!selectedConnectionUuid}>{versions.map((item) => <option key={item.uuid} value={item.uuid}>v{item.versionNumber} · {item.host}:{item.port}</option>)}</select></label>
      ...
      {form && <Drawer title={formTitle[form]} closeLabel={tr('close')} onClose={() => setForm(null)}>
        ...
        {form === 'version' && selectedConnection && <form className="topology-form" onSubmit={(event) => void submit('version', event, (data) => topologyApi.createVersion(projectUuid, selectedConnection.uuid, { driverReference: textValue(data, 'driverReference'), host: textValue(data, 'host'), port: Number(textValue(data, 'port')), serviceName: optionalValue(data, 'serviceName'), sid: optionalValue(data, 'sid'), tlsMode: optionalValue(data, 'tlsMode'), policyVersion: Number(textValue(data, 'policyVersion') || 1), secretReferenceUuid: optionalValue(data, 'secretReferenceUuid'), secretRole: optionalValue(data, 'secretRole') }), async () => { setVersions(await topologyApi.listVersions(projectUuid, selectedConnection.uuid)) })}><Field label={tr('driver')} name="driverReference" /><Field label={tr('host')} name="host" /><div className="topology-form-row"><Field label={tr('port')} name="port"><input name="port" type="number" min="1" max="65535" defaultValue="1521" required /></Field><Field label={tr('tlsMode')} name="tlsMode" optional /></div><Field label={tr('serviceName')} name="serviceName" optional /><Field label={tr('sid')} name="sid" optional /><Field label={tr('secret')} name="secretReferenceUuid" optional><select name="secretReferenceUuid" defaultValue=""><option value="">—</option>{resources.secrets.map((item) => <option key={item.uuid} value={item.uuid}>{item.name} · {item.provider}</option>)}</select></Field><Field label={tr('secretRole')} name="secretRole" optional /><Field label={tr('policyVersion')} name="policyVersion"><input name="policyVersion" type="number" min="1" defaultValue="1" required /></Field><Submit busy={busy === 'version'} c={c} /></form>}
        ...
        {form === 'environment' && <form className="topology-form" onSubmit={(event) => void submit('environment', event, (data) => topologyApi.createEnvironment(projectUuid, { code: textValue(data, 'code'), name: textValue(data, 'name'), risk: optionalValue(data, 'risk'), policyVersion: Number(textValue(data, 'policyVersion') || 1) }))}><Field label={tr('name')} name="name" /><Field label={tr('code')} name="code" /><Field label={tr('risk')} name="risk" optional><select name="risk" defaultValue=""><option value="">—</option><option value="LOW">LOW</option><option value="MEDIUM">MEDIUM</option><option value="HIGH">HIGH</option></select></Field><Field label={tr('policyVersion')} name="policyVersion"><input name="policyVersion" type="number" min="1" defaultValue="1" required /></Field><Submit busy={busy === 'environment'} c={c} /></form>}
        ...
        }}>{resources.physicalSchemas.map((item) => <option key={item.uuid} value={item.uuid}>{item.name}</option>)}</select></Field><Field label={tr('version')} name="connectionVersionUuid"><select name="connectionVersionUuid" required disabled={versionsLoading || versions.length === 0}>{versions.map((item) => <option key={item.uuid} value={item.uuid}>v{item.versionNumber} · {item.host}</option>)}</select></Field><Submit busy={busy === 'binding'} c={c} /></form>}
