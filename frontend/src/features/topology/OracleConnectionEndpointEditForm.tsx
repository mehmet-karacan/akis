...
export function OracleConnectionEndpointEditForm({ projectUuid, connectionUuid, connection, version, copy: c, onSaved, readOnly = false, footerActions }: Props) {
  ...
  return <form className="topology-form topology-endpoint-edit" onSubmit={(event) => void save(event)}>
    ...
    <div className="topology-form-row"><label className="topology-field"><span>{c.name} *</span><AntInput required value={name} onChange={(event) => setName(event.target.value)} /></label><label className="topology-field"><span>{c.code} *</span><AntInput required pattern="[A-Za-z][A-Za-z0-9_]{0,99}" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} /></label></div>
    <label className="topology-field"><span>{c.description}</span><AntInput.TextArea rows={2} value={description} onChange={(event) => setDescription(event.target.value)} /></label>
    <h3>{c.oracleConnectionDetails}</h3>
    <label className="topology-field"><span>{c.connectionMode} *</span><FormSelect value={draft.mode} onChange={(event) => update('mode', event.target.value as ConnectionVersionDraft['mode'])}><option value="JDBC">JDBC</option><option value="JNDI">JNDI</option></FormSelect></label>
    {draft.mode === 'JDBC' ? <>
      <div className="topology-form-row"><label className="topology-field"><span>{c.host} *</span><AntInput required value={draft.host} onChange={(e) => update('host', e.target.value)} /></label><label className="topology-field"><span>{c.port} *</span><AntInput required type="number" min="1" max="65535" value={draft.port} onChange={(e) => update('port', e.target.value)} /></label></div>
      <div className="topology-form-row"><label className="topology-field"><span>{c.connectionMethod} *</span><FormSelect value={draft.identifierType} onChange={(e) => update('identifierType', e.target.value as ConnectionVersionDraft['identifierType'])}><option value="SERVICE_NAME">{c.serviceName}</option><option value="SID">{c.sid}</option></FormSelect></label><label className="topology-field"><span>{draft.identifierType === 'SID' ? c.sid : c.serviceName} *</span><AntInput required value={draft.identifier} onChange={(e) => update('identifier', e.target.value)} /></label></div>
      <div className="topology-form-row"><label className="topology-field"><span>{c.username} *</span><AntInput required value={draft.username} onChange={(e) => update('username', e.target.value)} autoComplete="username" /></label><label className="topology-field"><span>{c.password} *</span><AntInput required={endpointChanged} type="password" value={draft.password} onChange={(e) => update('password', e.target.value)} autoComplete="new-password" /></label></div>
    </> : <label className="topology-field"><span>{c.jndiName} *</span><AntInput required value={draft.jndiName} onChange={(e) => update('jndiName', e.target.value)} /></label>}
    </fieldset>
    ...
    {!readOnly && <div className="topology-form-actions topology-form-actions--right">{footerActions}{!endpointChanged ? <ConnectionTestButton connectionUuid={connectionUuid} versionUuid={version.uuid} /> : <AntActionButton tone="secondary" className="topology-button topology-button--test" type="button" onClick={() => void test()} disabled={Boolean(busy)}>{busy === 'test' ? <LoaderCircle className="is-spinning" /> : <ShieldCheck />}{busy === 'test' ? c.testing : c.testDraftConnection}</AntActionButton>}<AntActionButton tone="primary" className="topology-button" type="submit" disabled={Boolean(busy)}>{busy === 'save' ? <LoaderCircle className="is-spinning" /> : <Save />}{busy === 'save' ? c.saving : c.saveConnection}</AntActionButton></div>}
  </form>
}
