...
export function ModelDetailPage() {
  ...
  return <section className="page-stack models-page"><Link className="connection-back-link" to={`/projects/${projectUuid}/models`}><ArrowLeft size={16} />{t('models.title')}</Link>
    <PageHeader eyebrow={`${model.code} · ${logical?.name ?? '—'}`} title={model.name} description={model.description ?? t('models.detailDescription')} actions={<Link className="button primary" to={`/projects/${projectUuid}/models/${modelUuid}/import`}><Download size={16} />{t('models.importMetadata')}</Link>} />
    <div className="model-detail-layout"><aside className="model-object-tree"><header><FolderTree size={18} /><strong>{t('models.dataObjects')}</strong><span>{objects.length}</span></header>{objects.length === 0 ? <p>{t('models.noDataObjects')}</p> : <ul>{objects.map((object) => <li key={object.uuid}><button className={selected?.uuid === object.uuid ? 'is-selected' : ''} onClick={() => setParams({ object: object.uuid })}><Boxes size={16} /><span><strong>{object.name}</strong><small>{submodels.find((item) => item.uuid === object.submodelUuid)?.name ?? object.type}</small></span></button></li>)}</ul>}</aside>
      <section className="model-object-detail">{!selected ? <AsyncState state="empty" title={t('models.noDataObjects')} /> : <><header><div><span>{selected.type}</span><h2>{selected.name}</h2><code>{selected.objectReference}</code></div><StatusBadge tone={selected.status === 'AKTIF' ? 'success' : 'neutral'}>{selected.status}</StatusBadge></header>{latest ? <><div className="metadata-evidence"><span>{t('models.discoveredAt')}</span><strong>{new Intl.DateTimeFormat(i18n.language, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(latest.discoveredAt))}</strong><span>{t('models.engineVersion')}</span><strong>{latest.engineVersion}</strong></div><DataObjectTable columns={latest.columns} /></> : <AsyncState state="empty" title={t('models.noSnapshot')} description={t('models.noSnapshotHint')} />}</>}</section>
    </div>
  </section>
}
