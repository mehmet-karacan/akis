...
export function ModelsPage() {
  ...
  return <section className="page-stack models-page">
    <PageHeader eyebrow={t('models.eyebrow')} title={t('models.title')} description={t('models.description')} actions={<Button tone="primary" icon={<Plus size={16} />} disabled={logicalSchemas.length === 0} onClick={() => setOpen(true)}>{t('models.create')}</Button>} />
    ...
    {loading ? <AsyncState state="loading" title={t('common.loading')} /> : models.length === 0 ? <AsyncState state="empty" title={t('models.empty')} description={t('models.emptyHint')} action={logicalSchemas.length ? <Button tone="primary" onClick={() => setOpen(true)}>{t('models.create')}</Button> : undefined} /> : <>
      ...
      <div className="model-table-wrap"><table><thead><tr><th>{t('models.name')}</th><th>{t('models.logicalSchema')}</th><th>{t('models.objectCount')}</th><th>{t('models.lastMetadataUpdate')}</th><th>{t('models.status')}</th></tr></thead><tbody>{filtered.map((model) => <tr key={model.uuid}>
        ...
        <td>{logicalSchemas.find((schema) => schema.uuid === model.logicalSchemaUuid)?.name ?? '—'}</td><td>{model.dataObjectCount ?? 0}</td><td>{formatDate(model.lastMetadataUpdate)}</td><td><StatusBadge tone={model.status === 'AKTIF' ? 'success' : 'neutral'}>{model.status}</StatusBadge></td>
      </tr>)}</tbody></table></div>
    </>}
    <Dialog open={open} title={t('models.create')} closeLabel={t('common.close')} busy={busy} onClose={() => setOpen(false)}><form onSubmit={(event) => void create(event)}>
