...
import { Activity, CheckCircle2, ChevronLeft, ChevronRight, Clock3, RefreshCw, Repeat2, Search, Timer, XCircle } from 'lucide-react'
...
export function RunsPage() {
  ...
  const [filterDraft, setFilterDraft] = useState({ status, environment, definitionType });
  ...
  return <section className="ops-page execution-page">
    ...
    <FilterSection><form className="run-filters" onSubmit={(event) => { event.preventDefault(); updateFilters({ query: queryDraft, statuses: filterDraft.status, environment: filterDraft.environment, definitionType: filterDraft.definitionType }) }}>
      ...
      {<AntActionButton tone="secondary" icon={<XCircle size={16} />} className="ops-button ops-button-secondary" type="button" onClick={() => { setFilterDraft({ status: '', environment: '', definitionType: '' }); setQueryDraft(''); setFromDraft(''); setToDraft(''); updateFilters({ query: '', statuses: '', environment: '', definitionType: '', from: '', to: '' }) }}>{t('clearFilters')}</AntActionButton>}</div>
    </form>
