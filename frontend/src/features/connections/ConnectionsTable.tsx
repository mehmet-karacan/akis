...
export function ConnectionsTable({ projectUuid, items, labels, onOpen }: {
  ...
  return <div className="connections-table-wrap"><DataGrid auditKind="connections" viewControls={false} className="connections-table">
    <thead><tr><th>{labels.provider}</th><th>{labels.connection}</th><th>{labels.host}</th><th>{labels.port}</th><th>{labels.service}</th><th>{labels.username}</th><th>{labels.status}</th><th>{labels.lastTest}</th><th className="numeric">{labels.physical}</th><th className="numeric">{labels.logical}</th><th><span className="sr-only">{labels.actions}</span></th></tr></thead>
    <tbody>{items.map(({ connection, displayedVersion, physicalSchemaCount, logicalSchemaCount }) => <tr key={connection.uuid}>
      ...
      <td className="connection-last-test">{displayedVersion?.testedAt ? new Intl.DateTimeFormat(i18n.language, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(displayedVersion.testedAt)) : labels.testRequired}</td>
