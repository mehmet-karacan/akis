...
export function PhysicalSchemasPage() {
  ...
  return <section className="page-stack schema-page"><Link className="connection-back-link" to={`/projects/${projectUuid}/connections/${connectionUuid}`}><ArrowLeft size={16} />{connection.name}</Link><PageHeader title={t('schemas.physicalTitle')} description={t('schemas.physicalDescription', { name: connection.name })} eyebrow={connection.code} />{error ? <div className="error-banner" role="alert">{error}</div> : null}{canManage && <section className="physical-create-panel"><form onSubmit={(event) => void create(event)}><label>{t('schemas.oracleUser')}<SuggestionInput suggestions={schemas}  value={schema} onChange={(event) => setSchema(event.target.value)} required placeholder="INNOVA_ODI" /></label><Button type="button" icon={<Database size={16} />} onClick={() => void loadSchemas()} busy={discovering} disabled={!usable}>{t('schemas.loadUsers')}</Button><Button type="submit" tone="primary" icon={<Plus size={16} />} busy={busy}>{t('schemas.addPhysical')}</Button></form><p>{usable ? t('schemas.manualUserHint') : t('schemas.testRequired')}</p><p>{t('schemas.metadataOnly')}</p></section>}{items.length === 0 ? <AsyncState state="empty" title={t('schemas.noPhysical')} /> : <div className="schema-list-table"><DataGrid auditKind="physical-schemas"><thead><tr><th>{t('schemas.name')}</th><th>{t('schemas.oracleUser')}</th></tr></thead><tbody>{items.map((item) => <tr key={item.uuid}><td><strong>{item.name}</strong></td><td><code>{item.schemaReference}</code></td></tr>)}</tbody></DataGrid></div>}</section>
}
