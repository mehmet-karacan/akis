...
export function ConnectionsPage() {
  ...
  const filtered = useMemo(() => catalog
    ...
    .filter((item) => status === 'ALL' || item.displayedVersion?.lifecycleStatus === status)
    ...
    .sort((a, b) => sort === 'code' ? a.connection.code.localeCompare(b.connection.code, i18n.language) : a.connection.name.localeCompare(b.connection.name, i18n.language)), [catalog, i18n.language, provider, query, sort, status])
  ...
  return <section className="page-stack connections-page">
    ...
    <FilterBar>
      ...
      <label><span className="sr-only">{t('connections.status')}</span><select value={status} onChange={(event) => updateParam('status', event.target.value)}><option value="ALL">{t('connections.allStatuses')}</option><option value="ACTIVE">{t('connections.active')}</option><option value="TESTED">{t('connections.tested')}</option><option value="DRAFT">{t('connections.draft')}</option></select></label>
