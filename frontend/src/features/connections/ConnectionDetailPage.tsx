...
export function ConnectionDetailPage() {
  ...
  return <section className="page-stack connection-detail-page">
    ...
    <PageHeader title={connection.name} description={connection.description ?? t('common.noDescription')} eyebrow={`${connection.databaseType} · ${connection.code}`} actions={canWrite ? <><Button icon={<Pencil size={16} />} onClick={() => setDialog('identity')}>{t('connections.edit')}</Button><Button tone="danger" icon={<Trash2 size={16} />} onClick={() => setDialog('delete')}>{t('connections.delete')}</Button></> : undefined} />
    ...
    <section id="revisions" className="connection-detail-section"><header><div><h2>{t('connections.revisions')}</h2><p>{t('connections.revisionsHint')}</p></div>{canWrite && <Button icon={<Plus size={16} />} onClick={() => setDialog('revision')}>{t('connections.newRevision')}</Button>}</header><div className="revision-list">{versions.map((version) => <Link key={version.uuid} to={`/projects/${projectUuid}/connections/${connectionUuid}/revisions/${version.uuid}`}><span><strong>r{version.versionNumber} · {version.mode}</strong><small>{endpointLabel(version)}</small></span><StatusBadge tone={version.lifecycleStatus === 'ACTIVE' ? 'success' : version.lifecycleStatus === 'TESTED' ? 'info' : 'warning'}>{lifecycleText(version.lifecycleStatus)}</StatusBadge></Link>)}</div></section>
