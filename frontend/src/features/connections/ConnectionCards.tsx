...
export function ConnectionCards({ items, view, onOpen }: { items: ConnectionCatalogItem[]; view: 'card' | 'list'; onOpen: (uuid: string) => void }) {
  ...
  return <div className={`ui-collection ui-collection--${view}`}>{items.map(({ connection, displayedVersion: version, physicalSchemaCount, logicalSchemaCount }) => {
    ...
    const fields: RecordField[] = [
      ...
      { icon: <UserRound />, label: tr ? 'Oluşturan' : 'Created By', value: connection.createdBy || (tr ? 'Kaydedilmemiş' : 'Not Recorded') },
    ...
    return <div key={connection.uuid} className="connection-record-card" data-connection-uuid={connection.uuid} role="button" tabIndex={0} onClick={() => onOpen(connection.uuid)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpen(connection.uuid) } }}>
