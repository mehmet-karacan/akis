...
export const ConnectionCards = memo(function ConnectionCards({ items, view, onOpen, audit }: { audit?: ReturnType<typeof useRecordAudit>; items: ConnectionCatalogItem[]; view: 'card' | 'list'; onOpen: (uuid: string) => void }) {
  ...
  return <div className={`ui-collection ui-collection--${view}`}>{items.map(({ connection, displayedVersion: version, physicalSchemaCount, logicalSchemaCount }) => {
    ...
    return <Card key={connection.uuid} className="connection-record-card" data-connection-uuid={connection.uuid}>
      <header><span className="ui-collection-icon"><DatabaseProviderIcon databaseType={connection.databaseType} /></span><div className="connection-record-identity"><strong>{connection.name}</strong><small>{connection.databaseType === 'ORACLE' ? 'Oracle' : connection.databaseType}</small></div><span title={version?.testedAt ? `${tr ? 'Son test' : 'Last test'}: ${new Intl.DateTimeFormat(i18n.language, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(version.testedAt))}` : t('connections.notTested')} className={`connection-header-state ${tested ? 'connection-test-pass' : 'connection-test-pending'}`}>{tested ? <CheckCircle2 size={12} /> : <CircleAlert size={12} />}{tested ? (tr ? 'Test Başarılı' : 'Test Passed') : t('connections.notTested')}</span></header>
      ...
      <footer><time dateTime={version?.testedAt ?? undefined} title={tr ? 'Son test zamanı' : 'Last tested'}>{version?.testedAt && <Clock3 size={14} />}<span>{tr ? 'Son Test' : 'Last Test'}:</span>{version?.testedAt ? new Intl.DateTimeFormat(i18n.language, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(version.testedAt)) : t('connections.notTested')}</time><div className="connection-card-actions"><ConnectionTestButton connectionUuid={connection.uuid} versionUuid={version?.uuid} /><Button type="button" className="connection-card-action" icon={<Pencil size={16} />} aria-label={`${tr ? 'Düzenle' : 'Edit'}: ${connection.name}`} onClick={() => onOpen(connection.uuid)} /></div></footer>
    </Card>
  })}</div>
})
