...
export function QueryFilter({ onApply, placeholder }: { onApply: (query: string) => void; placeholder: string }) {
  ...
  return <FilterSection><form className="ui-query-form" onSubmit={(event) => { event.preventDefault(); onApply(draft.trim()) }}>
    <label><span className="ui-query-label"><Search size={15} aria-hidden="true" />{tr ? 'Ad veya Kod' : 'Name or Code'}</span><Input type="search" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={placeholder} /></label>
