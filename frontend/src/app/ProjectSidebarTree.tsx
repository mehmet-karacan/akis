...
export function ProjectSidebarTree({ folders, definitions, selectedUuid, loading, failed, onNavigate, onRetry, projectUuid }: Props) {
  ...
  const folderNode = (folder: FolderTreeNode): TreeDataNode => ({ key: folder.uuid,
    title: title(folder.name, <Folder size={16} />, () => toggle(folder.uuid), canWrite ? [{ key: 'folder', label: shellT('nav.createSubfolder'), icon: <FolderPlus size={15} />, onClick: () => navigate(`/objects?createFolder=${encodeURIComponent(folder.uuid)}`) }] : []),
  ...
  const nodes: TreeDataNode[] = [
    { key: 'flows', title: title(shellT('nav.flows'), <Workflow size={16} />, () => toggle('flows'), canWrite ? [{ key: 'root', label: shellT('nav.createRootFolder'), icon: <FolderPlus size={15} />, onClick: () => navigate('/objects?createFolder=') }] : []), children: [...tree.map(folderNode), ...(active.some(item => !item.folderUuid && FLOW_GROUPS.some(group => group.type === item.type)) ? [{ key: 'unfiled', title: title(shellT('nav.unfiled'), <Folder size={16} />, () => toggle('unfiled')), children: flowGroups(null) }] : [])] },
    { key: 'components', title: title(shellT('nav.commonComponents'), <Blocks size={16} />, () => toggle('components'), componentActions, undefined, shellT('nav.addComponent')), children: COMPONENT_TYPES.map(type => ({ key: type, title: title(t(definitionTypeKey[type]), <DefinitionTypeIcon type={type} />, () => toggle(type), componentActions?.filter(item => item?.key === type)), children: active.filter(item => item.type === type).map(objectNode) })) },
  ]
