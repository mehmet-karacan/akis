...
export function ConnectionsSubnavigation({ hasPendingChanges, onNavigate }: { hasPendingChanges: boolean; onNavigate: (path: string) => void }) {
  ...
  return <div className="workspace-subnavigation workspace-link-list"><p className="nav-label">{t('nav.connectionsContext')}</p>{links.map(({ path, key, icon: Icon }) => {
    ...
    return <NavLink key={key} end={path === '/connections'} className={({ isActive }) => `workspace-subnav-link ${isActive ? 'active' : ''}`} to={target} data-nav={path.slice(1)} onClick={(event) => { if (hasPendingChanges || !(event.ctrlKey || event.metaKey || event.shiftKey || event.altKey)) { event.preventDefault(); onNavigate(target) } }}><Icon /><span>{t(key)}</span></NavLink>
  })}</div>
}
