...
Temiz baseline, araştırma raporundaki ortak fiziksel şema sözleşmesini uygular:

- her tabloda iç ilişkiler için `id BIGINT GENERATED BY DEFAULT AS IDENTITY`
  primary key bulunur,
- her tabloda API ve dış aktarım kimliği olarak `uuid UUID NOT NULL UNIQUE`
  bulunur,
- foreign key alanları `BIGINT` olup hedef tablonun iç `id` alanına bağlanır,
- API kullanıcıdan `id` kabul etmez ve dışarıya `uuid` verir,
- değişebilir tablolarda oluşturma/güncelleme kullanıcıları, zamanları ve
  `versiyon_no` eksiksiz tutulur,
- yüksek hacimli append-only tablolar kendi E audit sözleşmesini kullanır.
...
carry `proje_id`; system-scoped assignments do not. Built-in Developer,
